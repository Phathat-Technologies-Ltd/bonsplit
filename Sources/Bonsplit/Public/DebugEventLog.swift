...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "phatmux.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["PHATMUX_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["PHATMUX_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/phatmux-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["PHATMUX_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("phatmux-debug-") {
                return "/tmp/\(socketBase).log"
            }
        }

        if let bundleId = Bundle.main.bundleIdentifier,
           bundleId != "com.phatmux.app.debug" {
            return "/tmp/phatmux-debug-\(sanitizePathToken(bundleId)).log"
        }

        return "/tmp/phatmux-debug.log"
    }
